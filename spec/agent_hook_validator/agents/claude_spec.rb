...
RSpec.describe AgentHookValidator::Agents::Claude do
  ...
  describe '#call' do
    it 'executes claude CLI and returns stdout' do
      ...
      stdout = instance_double(IO, read: "Review looks good\n", closed?: true)
      ...
      allow(Open3).to receive(:popen3).with('claude', '-p', '-')
    ...
    it 'raises AgentExecutionError on non-zero exit' do
      ...
      stderr = instance_double(IO, read: 'command not found', closed?: true)
